...
and get rid of them.  LectroTest will search the haystack for you --
...
a "thumbs up" (true) if the point is free of needles and a "thumbs
...
of the_thing_we_are_testing() must be non-negative.

=head2 Put them together to get a Property

The generator bindings and needle recognizer are combined into a
LectroTest::Property:
...
strictly required, it's an excellent practice.
...
the_thing_we_are_testing is non_negative."  See?  That's not too bad.

=head2 Testing whether your Properties hold

When you define properties for your software, just add them to
...
What this means is that at the point (I<x>=-4, I<c>="R") in the
haystack, there is a needle (i.e., you're property didn't hold).
...
[To do: write the step-by-step tutorial examples.  For now,
